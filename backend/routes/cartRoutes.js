...
//@route PUT /api/cart
//@desc Update the quantity of product in cart for guest/logged in user
//@access public
...
//@desc Delete product from cart for guest/logged in user
//@access public
router.delete("/", async (req, res) => {
    const {productId, size, color, guestId, userId} = req.query;
    // console.log("DELETE hit", req.body); 
    //   console.log("DELETE /api/cart", req.query); // 🔍 for debugging
    ...
    try {
        ...
        const productIndex = cart.products.findIndex(
            ...
            && p.size.toLowerCase() === size.toLowerCase() && p.color.toLowerCase() === color.toLowerCase());
...
//@ GET /api/cart
//@desc Get guest/logged-in user cart
//@access public
    ...
    try {
        ...
        if(guestCart){
            ...
            if(userCart){
                ...
                if(productIndex > -1){
                    //if the item exists in the user cart, udate the quantity
            ...
            if(userCart) {
                //guest cart has already merge , return user cart
